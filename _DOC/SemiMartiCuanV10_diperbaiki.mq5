...
input double InpOrderGapPrice    = 2.5;   // BARU: jarak layer dalam SATUAN HARGA (0 = pakai rumus pip
                                          // lama). Untuk XAUUSD 2-digit isi 2.5; rumus pip menghasilkan
                                          // 0.25 di sana - lihat catatan panjang di TryEnter().
...
input double InpGlobalSL_USD     = 70.0;  // DIUBAH dari 0 (= tanpa SL sama sekali) atas permintaan user.
                                          // Tanpa ini, satu tren melawan = kerugian tanpa batas bawah.
input double InpMaxLayerLot      = 0.02;  // BARU. Dulu cap 0.03 DI-HARDCODE di dalam TryEnter().
                                          // Dijadikan input supaya bisa diuji dan dibatasi user.
...
input bool   InpDebug            = false;  // DIMATIKAN: dengan true, EA mencetak tiap tick dan
                                            // Strategy Tester merangkak (5 hari data = 4 menit CPU).
...
void TryEnter(const ENUM_ORDER_TYPE orderType)
  {
   ...
   // ---------------------------------------------------------------------------
   //  PERBAIKAN 2026-08-11 - jarak layer sekarang bisa diberikan dalam SATUAN HARGA.
   //
   //  BUG ASLINYA: pipMul = (digits > 3) ? 10 : 1. Rumus itu benar untuk forex
   //  5-digit (25 * 0.00001 * 10 = 0.0025 = 25 pip), tapi XAUUSD di FBS punya
   //  digits = 2, jadi pipMul jatuh ke 1 dan baseGap = 25 * 0.01 * 1 = 0.25.
   //  "25 pip" berubah jadi 25 SEN emas. Emas bergerak $30-60 sehari, jadi
   //  ketiga layer terisi dalam gerakan $1 - martingale-nya kehilangan jaraknya
   //  sama sekali dan cuma melipatgandakan ukuran seketika.
   //
   //  "Pip" artinya berbeda-beda per simbol dan itu sumber ambiguitasnya. Jadi
   //  jalan keluarnya bukan menambal pipMul, tapi memberi jalan yang TIDAK bisa
   //  disalahartikan: InpOrderGapPrice dalam satuan harga. Untuk emas isi 2.5
   //  (setara 25 pip emas konvensional). Kalau 0, rumus pip lama tetap dipakai
   //  supaya EA ini masih jalan apa adanya di forex.
   // ---------------------------------------------------------------------------
   double baseGap;
   if(InpOrderGapPrice > 0.0)
      baseGap = InpOrderGapPrice;
   else
     {
      double pipMul = (digits == 3 || digits == 5) ? 10.0 : 1.0;
      baseGap = InpOrderGapPips * point * pipMul;
     }
   ...
   // Lot sizing: 0.01 * (nextLayer + 1), cap sekarang dari input (dulu hardcoded 0.03)
   double rawLot = InpStartLot * (double)(sameCount + 1);
   if(rawLot > InpMaxLayerLot) rawLot = InpMaxLayerLot;
