...
while ($true) {
    ...
    if ($ok) {
        ...
        if (((Get-Date) - $lastHb).TotalMinutes -ge $HeartbeatMin) {
            # ringkasan aktivitas vision (baca file saja, tidak menyentuh MT5)
            $vsum = "vision_journal: belum ada entri"
            if (Test-Path $VisionJrn) {
                $vlines = Get-Content $VisionJrn
                $last = ($vlines | Select-Object -Last 1)
                if ($last.Length -gt 180) { $last = $last.Substring(0,180) + "..." }
                $vsum = "vision entries=$($vlines.Count); terakhir: $last"
            }
            J "HB " "Sehat (heartbeat). $vsum"
            $lastHb = Get-Date
        }
    } else {
        $fail++
